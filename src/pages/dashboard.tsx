...
import { getPublications } from 'apiSdk/publications';
import { PublicationInterface } from 'interfaces/publication';
import { getPapers } from 'apiSdk/papers';
import { PaperInterface } from 'interfaces/paper';
import { getReviews } from 'apiSdk/reviews';
import { ReviewInterface } from 'interfaces/review';

import { PublicationListPage as ShowCaseList } from 'pages/publications';

function DashboardPage() {
  const { session } = useSession();

  const publicationStat = useSWR<PaginatedInterface<PublicationInterface>>(
    () => `/publications`,
    () => getPublications({ limit: 1, offset: 0 }),
  );

  const paperStat = useSWR<PaginatedInterface<PaperInterface>>(
    () => `/papers`,
    () => getPapers({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Publications',
      result: publicationStat?.data?.totalCount,
      isLoading: publicationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Papers',
      result: paperStat?.data?.totalCount,
      isLoading: paperStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Reviews',
